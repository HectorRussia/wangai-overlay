...
impl WorkerManager {
    pub fn start(&self, app: AppHandle, settings: &AppSettings) -> Result<()> {
        ...
        command
            ...
            .arg(&worker_path)
            .arg("--vad-threshold")
            .arg(settings.vad.vad_threshold.to_string())
            .arg("--silence-ms")
                    ...
                    match serde_json::from_str::<WorkerEvent>(&line) {
                        Ok(event) => {
                            ...
                            tauri::async_runtime::spawn(async move {
                                pipeline::handle_worker_event(app, event).await;
                            });
                        }
                        Err(error) => {
    ...
    pub fn send_audio(&self, stream: StreamKind, samples: Vec<f32>) -> bool {
        let sender = self
            .sender
            .read()
            .expect("worker sender lock poisoned")
        ...
        match sender.try_send(WorkerCommand::Audio(stream, samples)) {
            Ok(()) => true,
...
fn write_frame(mut writer: impl Write, message: WorkerCommand) -> std::io::Result<()> {
    let (kind, stream, payload) = match message {
        WorkerCommand::Audio(stream, samples) => {
            let mut payload = Vec::with_capacity(samples.len() * 4);
            for sample in samples {
                payload.extend_from_slice(&sample.to_le_bytes());
            }
            (FRAME_AUDIO, stream.id(), payload)
        }
        WorkerCommand::Reset(stream) => (FRAME_RESET, stream.id(), Vec::new()),
        WorkerCommand::Finalize(stream) => (FRAME_FINALIZE, stream.id(), Vec::new()),
        WorkerCommand::Shutdown => (FRAME_SHUTDOWN, 0, Vec::new()),
    };
...
mod tests {
    ...
    fn audio_frame_has_expected_header_and_payload() {
        ...
        write_frame(
            &mut data,
            WorkerCommand::Audio(StreamKind::Game, vec![0.5, -0.5]),
        )
        .expect("write");
