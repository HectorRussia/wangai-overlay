    ...
    {activeTab === "advanced" && <>
      ...
      {advancedSection === "ai" && <section className="space-y-4">
        <Card title="บริการ AI" icon={<Cloud />} subtitle="ใช้บริการกลาง ไม่ต้องใส่ API key หรือเลือกโมเดลเอง">
          <p role="status" aria-label="สถานะบริการ AI" className="mb-4 text-sm text-[#76dda0]">{runtime.aiService.message}</p>
      ...
      {advancedSection === "controls" && <section className="space-y-4">
        <Card title="Hotkeys" icon={<KeyRound />} subtitle="F8 ฟังแอปที่เลือก · F9 ตอบกลับด้วยไมค์"><div className="grid gap-3 md:grid-cols-2">{Object.entries(hotkeys).map(([key, value]) => <label className="text-sm" key={key}>{key}<input className={`${input} mt-1`} value={value} onChange={(event) => setHotkeys({ ...hotkeys, [key]: event.target.value })} /></label>)}</div><button className={`${primary} mt-4`} onClick={() => void run("hotkeys", () => api.updateHotkeys(hotkeys), "บันทึก hotkeys แล้ว")}>บันทึก Hotkeys</button></Card>
        <Card title="Overlay" icon={<SlidersHorizontal />} subtitle="รูปแบบหน้าต่างคำแปล">
          <div className="grid gap-5 md:grid-cols-2">
            <Slider label="Opacity" min={0.2} max={1} step={0.05} value={overlay.opacity} display={`${Math.round(overlay.opacity * 100)}%`} onChange={(value) => setOverlay({ ...overlay, opacity: value })} />
            <Slider label="ขนาดตัวอักษร" min={0.7} max={1.8} step={0.05} value={overlay.fontScale} display={`${Math.round(overlay.fontScale * 100)}%`} onChange={(value) => setOverlay({ ...overlay, fontScale: value })} />
            <Slider label="จำนวนข้อความ" min={1} max={5} step={1} value={overlay.maxItems} display={`${overlay.maxItems}`} onChange={(value) => setOverlay({ ...overlay, maxItems: value })} />
          </div>
          <button className={`${primary} mt-4`} onClick={() => void run("overlay", () => api.updateOverlay(overlay), "บันทึก Overlay แล้ว")}>บันทึก Overlay</button>
        </Card>
      </section>}
    </>}
