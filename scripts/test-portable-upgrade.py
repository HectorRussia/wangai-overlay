"""Opt-in real Portable version-pair acceptance harness (no installer/AI).
...
def main():
    ...
    parser.add_argument('--from-version',default='0.3.0');parser.add_argument('--to-version',default='0.4.0')
    args=parser.parse_args()
    import re
    old,new=args.from_version,args.to_version
    for version in (old,new):
        if not re.fullmatch(r'(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)',version):parser.error('Use numeric major.minor.patch versions')
    if tuple(map(int,new.split('.'))) <= tuple(map(int,old.split('.'))):parser.error('Target must be newer than baseline')
    ...
    launcher_hashes={v:read(artifacts/f'v{v}/package-manifest.json')['files']['WANGAI.exe']['sha256'] for v in (old,new)}
    assert launcher_hashes[old]!=launcher_hashes[new],'Build distinct launcher versions to exercise launcher replacement'
    ...
    control={old:'smoke',new:'smoke','_headless':True};write(root/'Data/test-control.json',control)
    signature=(artifacts/f'v{new}/WANGAI_{new}_x64-update.zip.sig').read_text().strip()
    manifest={'version':new,'notes':'Isolated Portable QA','platforms':{'windows-x86_64':{'url':f'http://127.0.0.1:19438/v{new}/WANGAI_{new}_x64-update.zip','signature':signature}}}
    downloads=[]
    ...
    try:
        ...
        prepared=subprocess.run([str(artifacts/f'v{old}/WANGAI_{old}_x64-portable.exe'),'--test-prepare',str(root)],env=env,timeout=600)
        assert prepared.returncode==0, f'Initial preparation failed; inspect {root / "Data/test-startup.json"} and test-report-{old}.json'
        before=wait(lambda:read(root/f'Data/test-report-{old}.json'))
        ...
        assert file_hash(root/'WANGAI.exe')==launcher_hashes[old]
        ...
        control[old]='manual' if args.manual_confirmation else 'upgrade';control[new]='fail-ready' if args.rollback else 'smoke';write(root/'Data/test-control.json',control)
        ...
        if args.rollback:
            wait(lambda:read(root/'App/active.json').get('current')==new,600)
            assert file_hash(root/'WANGAI.exe')==launcher_hashes[new]
            replaced_pid=read(root/f'Data/test-report-{old}.json')['pid']
            control[old]='smoke';write(root/'Data/test-control.json',control)
            wait(lambda:read(root/'App/active.json').get('current')==old,180)
            wait(lambda:read(root/'Data/portable-test-error.json'),180)
            after=wait(lambda:(report if (report:=read(root/f'Data/test-report-{old}.json')).get('pid') not in (None,replaced_pid) else None),180)
            assert '90' in read(root/'Data/portable-test-error.json')['error']
        else:
            after=wait(lambda:read(root/f'Data/test-report-{new}.json'),900)
            wait(lambda:not (root/'App/transaction.json').exists())
            assert read(root/'App/active.json')=={'format':1,'current':new,'previous':old}
        ...
        expected_launcher=launcher_hashes[old if args.rollback else new]
        assert file_hash(root/'WANGAI.exe')==expected_launcher,'Launcher was not replaced/restored correctly'
        old_report=read(root/f'Data/test-report-{old}.json')
        assert not alive(old_report['pid']) and not alive(old_report['workerPid'])
