...
def frame(kind: int, stream: int, samples=None) -> bytes:
    payload = b"" if samples is None else np.asarray(samples, dtype="<f4").tobytes()
    body = struct.pack("<BBH", kind, stream, 0) + payload
    return struct.pack("<I", len(body)) + body


class WorkerIntegrationTests(unittest.TestCase):
    def test_mock_microphone_speech_boundaries(self):
        ...
        try:
            ...
            process.stdin.write(frame(1, 2, np.ones(8_000, dtype=np.float32) * 0.05))
            process.stdin.write(frame(3, 2))
            process.stdin.flush()
            ...
            self.assertTrue(all(event["stream"] == "microphone" for event in boundaries))
        finally:
            if process.stdin:
